...
import { addFileToSubject, fetchSubjects, submitTest } from '../../store/actions/subjectActions';
...
class TestingCatalog extends Component {
    ...
    inputChangeSubject (value) {
        console.log(value);
        this.state.testing.subject = value;
    }
    openAddTestModal = () => {
    ...
    closeAddTestModal = () => {
        this.setState({
            addTestModal: false, subject: '', testingTitle: '', questions: [],
            testing: {
                title: '',
                subject: '',
                questions: [],
    ...
    submitFormHandler = event => {
        ...
        console.log(this.state.testings, this.state)
    ...
    submitVariants = () => {
        ...
        if (this.state.answerTitle.length > 0) {
            this.setState({
                answerVariants: [...answerVar, this.state.answerTitle],
    ...
    submitFormHandl = event => {
        ...
        this.props.submitTest(42, { answerFlags: this.state.answerFlags, answerVariants: this.state.answerVariants});
    };
    ...
    deleteAnswer = (id) => {
        this.setState({
            answerVariants: this.state.answerVariants.filter((item, ind) => ind !== id)
        })
    }
    ...
    deleteQuestion = (id) => {
        this.setState({
            testing: {
                questions: this.state.testing.questions.filter((item, ind) => ind !== id)
            }
        })
    }
                                ...
                                <DialogContent>
                                    <form className='add-notice-form'>
                                            ...
                                            <div className='test-header'>
                                                <FormControl variant='outlined'>
                                                    <Select
                                                        ...
                                                        onChange={(e) => this.inputChangeSubject(e.target.value)}
                                                        inputProps={{
                                                            name: 'subject',
                                                        }}
                                                    >
                                            ...
                                            : <div className='test-header-upd'>
                                                <div>
                                                    ...
                                                    <p>дисциплина : {this.props.subjects.map(subject => {
                                                        if (this.state.testing.subject === subject._id) {
                                        ...
                                        <ol className='questions-list'>{/*предпросмотр*/}
                                            {this.state.testing.questions.map((el, idx) => {
                                                return <div>
                                                    <li><h3 className='answers2'> {el.title}
                                                        <span onClick={() => this.openDeleteModal()}>
                                                ...
                                                <form className='add-question'>
                                                    ...
                                                    <ol>
                                                        {this.state.answerVariants.map((el, index) => {
                                                            if (this.state.answerFlags[index] === true) {
                                                                return <li className='answers'> {el} (Правильный ответ)</li>
                                                            } else {
                                                                return <li className='answers'> {el}

                                                                    {/* <span onClick={() => this.deleteAnswer(index)} >
                                                                        <img className='delete_icon' src={deleteIcon} />
                                                                    </span> */}
                                                                    <span onClick={() => this.openDeleteModal()} >
                                                                        <img className='delete_icon' src={deleteIcon} />
                                                    ...
                                                    {this.state.questionType === 'more' ? <div>
                                                        ...
                                                        {this.state.required === true &&
                                                            <span className='req'>Введите вариант ответа</span>
                                                        }
                                                    </div> : null}
...
const mapDispatchToProps = dispatch => ({
    ...
    submitTest: (subjectId, subjectData) => dispatch(submitTest(subjectId, subjectData)),
});
